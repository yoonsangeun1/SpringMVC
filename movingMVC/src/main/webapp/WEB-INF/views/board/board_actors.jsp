<%@ page contentType="text/html; charset=UTF-8"%>
<%@ include file="../include/header.jsp"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>배우 모집</title>
</head>
<body>
 <div id="bActors_body"> <%--전체 div --%>
 
  <div id="bActors_community"> <%--왼쪽 업데이트 사항. 공지,이벤트 등 div --%>
		 <div class="bActors_community_tit"> <%--업데이트--%>
		  	<h1>커뮤니티</h1>
		 </div> <%--업데이트 --%>
		 
		  <div class="bActors_community_list"> <%--업데이트 밑에 리스트들 div --%>
		   <ul>
		   
		    <li class="bActors_community_list_actors">
		     <a href="board_actors">배우모집</a>
		    </li>
		    
		    <li>
		     <a href="board_contest">시나리오 공모</a>
		    </li>
		    
		    <li>
		     <a href="board_free">자유게시판</a>
		    </li>
		    
		    <li>
		     <a href="board_qna">QnA</a> <%--QnA 작업해야 됨 --%>
		    </li>
	    
		   </ul>
		  </div> <%--업데이트 밑에 리스트들 div --%>
		
		</div> <%--커뮤니티 배우모집,시나리오 공모 등 div --%>
		
  <div class="bActors_panel" > <%--연기자 프로필 주의사항 --%>
   <h3>연기자 프로필</h3>
    <div id="bActors_panelbody"> <%--주의사항 글 --%>
     -일반 회원이 글쓰기 가능한 연기자 프로필 페이지입니다.
     <br/>
     -이곳에 등록된 연기자를 MOVING에서 매니지먼트 하는게 아닙니다. 직접 연락하시면 됩니다.
     <br/>
     -에이전시나 매니지먼트 업체의 '프로필 보내달라'는 댓글은 자제해주세요. 삭제대상입니다.
    </div> <%--주의사항 글 --%>
  </div> <%--연기자 프로필 주의사항 --%>
  

   <div id="bActors_list"> <%-- 배우 목록들 --%>
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model01.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	추정훈
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model02.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	임정민
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model03.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	안현진
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model04.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	박창현
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model05.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	김민준
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model06.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	송민혁
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model07.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	박정언
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model08.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	진시원
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model09.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	강정호
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model10.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	김용준
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model11.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	이경오
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model12.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	박서현
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model13.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	윤선
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model14.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	백민서
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model15.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	유창한
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model16.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	현동현
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model17.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	노수민
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model18.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	강진철
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model19.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	이승훈
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model20.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	정유현
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model21.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	김준경
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model22.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	조용진
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model23.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	이승우
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>
    
    <div id="bActors_model"> <%--배우들 네모 전체 div --%>
     <div> <%--배우들 사진 --%>
      <a href="#">
       <img src="resources/images/model24.jpg" />
      </a>
     </div> <%--배우들 사진 --%>
      <div id="bActors_name"> <%--배우들 성함 --%>
       <a href="#">
        <img src="resources/images/character.gif" id="character"/>
         	정한수
       </a>
      </div> <%--배우들 성함 --%>
    </div> <%--배우들 네모 전체 div --%>  
   </div> <%-- 배우 목록들 --%> 
   
   <div> <%-- 밑 전체 div --%>
    <div id="bActors_writing"> <%-- 글 쓰기 폼 div --%>
      <input type="button" class="bActors_writing_btn button
       button_c9d8ce2 button_f12 button_p1024 button_r4"
      value="글 쓰기" />
    </div> <%-- 글 쓰기 폼 div --%>
    
        <div id="clear"></div>
    
    <div id="bActors_page"> <%--페이지 순번 --%>
		 <a href="board_actors">첫페이지</a>
		 <a href="board_actors">&lt;&lt;</a>
		 <a href="board_actors">1</a>
		 <a href="board_actors">2</a>
		 <a href="board_actors">3</a>
		 <a href="board_actors">4</a>
		 <a href="board_actors">5</a>
		 <a href="board_actors">6</a>
		 <a href="board_actors">7</a>
		 <a href="board_actors">8</a>
		 <a href="board_actors">9</a>
		 <a href="board_actors">10</a>
		 <a href="board_actors">&gt;&gt;</a>
		 <a href="board_actors
		 ">끝페이지</a>
		</div> <%--페이지 순번 --%>
    
		
  <div id="bActors_search"> <%--검색 구간 전체 div --%>
   <select name="bActors_combo" id="bActors_combo">
    <option value="title_content">제목+내용</option>
    <option value="title">제목</option>
    <option value="content">내용</option>
    <option value="user_name">이름</option>
    <option value="tag">태그</option>
   </select>
   
   <div id="bActors_input"> <%--텍필,버튼 --%>
    <input id="search_text" name="search_text" size="20" />
    <input type="submit" class="search_btn button_wce8e8e8" value="검색" />
   </div> <%--텍필,버튼 --%>
   
  </div> <%--검색 구간 전체 div --%>
   
  </div> <%--프로필 전체 감쌀 div --%>
 </div> <%--  전체 div --%>

<%@ include file="../include/footer.jsp"%>









