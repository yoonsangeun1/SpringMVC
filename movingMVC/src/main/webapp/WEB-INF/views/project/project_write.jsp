...
<div id="fWrite_wrap" style="padding-top: 90px">
	...
	<div id="fWrite_title">
		...
		<ul class="menu">
			...
			<li><c:if test="${projectPostVO.statusCode == 0 }">
					<button type="button" class="btn"
						...
						검토 요청하기</button>
				</c:if> <c:if test="${projectPostVO.statusCode == 1 }">
					<button type="button" class="btn"
		...
		<div id="fWrite_Alert" style="float: left; width: 100%">
			<div style="color: red; border: 1px solid red; padding: 10px">
				<p align="center">알림: 30분 동안 입력 또는 작성 단계별 저장이 진행되지 않으면 자동으로 로그아웃
					되며, 저장하지 않은 입력값은 반영 되지 않으니 주의하세요.</p>
			</div>
		</div>
		...
		<div id="fWrite_cont" class="menu-option on">
			<form method="post"
				action="/moving.com/project/write_ok?id=${projectPostVO.id }"
				onsubmit="" enctype="multipart/form-data">
				<table border="1" id="fWrite_cont_table" cellspacing="0">

					<tr>
						<td id="fWrite_cont_no">프로젝트 번호<br />
							<p style="font-weight: initial;">담당자와 소통은 프로젝트 번호로 진행됩니다.</p>
						</td>
						<td><input type="hidden" name="id"
							value="${projectPostVO.id }"> <input type="hidden"
							name="userId" value="${sessionUser.id }"> 프로젝트 번호 :
							${projectPostVO.id } <br />
							http://192.168.0.81:8084/moving.com/project/content?id=${projectPostVO.id }로
							프로젝트 개설 이후 접근 가능합니다.</td>
					</tr>

					<tr style="padding: 10px;">
						<td id="fWrite_cont_phone" rowspan="2">연락처
							<p style="font-weight: initial;">주요 안내를 받으실 이메일과 휴대폰 번호를 등록해
								주세요. 정보 변경은 회원정보설정에서 가능합니다.</p>
						</td>

						<td><input type="text" id="email" name="email"
							placeholder="이메일 주소를 입력하세요" value="${email }" size="50" /> <input
							type="button" id="checkEmail" name="checkEmail" value="인증하기" />
						</td>
					</tr>

					<tr>
						<td><input type="text" id="phone" name="phone"
							placeholder="전화 번호를 입력하세요" value="${phone }" size="50" /> <input
							type="button" id="checkPhone" name="checkPhone" value="인증하기" />
						</td>
					</tr>
 					...
 					onsubmit="" enctype="multipart/form-data"> -->
						<td id="fWrite_cont_image">대표 이미지<br />
							<p style="font-weight: initial;">
								메이커와 리워드가 함께 있거나, 프로젝트의 성격이 한눈에 드러나는 사진이 좋습니다. <a href="#">가이드</a>를확인하세요.
							</p>
						</td>
						...
						<td>
							<div class="select_img">
								<c:if test="${!empty projectPostVO.thumbnailImage  }">
									<img src="${projectPostVO.thumbnailImage}" width="370"
										height="220" />
								</c:if>
								<c:if test="${empty projectPostVO.thumbnailImage  }">
									<img src="" />
								</c:if>
							</div> <input type="file" name="thumbnailImage" id="titleImg" /> <script>
								//파일 업로드 시 해당 화면에 바로 띄워 지게 해줌.
								$("#titleImg")
										.change(
												function() {
													if (this.files
					...
					<tr>
						<td id="fWrite_cont_title">프로젝트 제목<br />
							<p style="font-weight: initial;">프로젝트 성격과 리워드를 짐작할 수 있게 간결하고
								직관적으로 작성해주세요.</p>
						</td>

						<c:if test="${empty projectPostVO.title }">
							<td><input name="title" id="title" size="20"
								placeholder="프로젝트 제목을 입력하세요" /></td>
						</c:if>

						<c:if test="${!empty projectPostVO.title}">
							<td><input name="title" id="title" size="20"
								value="${projectPostVO.title }" /></td>
						</c:if>
					</tr>
					...
					<tr>
						...
						<c:if test="${empty projectPostVO.introduce }">
							<td><input name="introduce" id="introduce" size="50"
								placeholder="프로젝트 한 줄 소개글을 입력하세요" /></td>
						</c:if>

						<c:if test="${!empty projectPostVO.introduce }">
							<td><input name="introduce" id="introduce" size="50"
								value="${projectPostVO.introduce }" /></td>
						</c:if>
					...
					<tr>
						<td id="fWrite_cont_hashtag">해쉬 태그<br />
							<p style="font-weight: initial;">검색, 알림 등에 사용되는 짧은 제목도 함께
								작성해주세요.</p>
						</td>
						<td><input name="hashtag" id="hashtag" size="50"
							placeholder="태그명은 쉼표로 구분해주세요(ex. 태그1, 태그2 )" /></td>
					</tr>

					<tr>
						<td id="fWrite_cont_targetMoney">목표 금액<br />
							<p style="font-weight: initial;">마감일 자정 기준 목표금액 미달성 시, 펀딩은
								취소됩니다. (리워드 평균 목표금액 : 300만원)
							<p></td>

						<c:if test="${empty projectPostVO.targetPrice }">
							<td><input name="targetPrice" id="goalMoney" size="20"
								size="50" placeholder="목표 금액을 입력해주세요" /> 원</td>
						</c:if>
						<c:if test="${!empty projectPostVO.targetPrice }">
							<td><input name="targetPrice" id="goalMoney" size="20"
								value="${projectPostVO.targetPrice }" /> 원</td>
						</c:if>
					</tr>
						...
						<td><select name="codeNo">
								<c:if test="${!empty projectPostVO.codeNo}">
									<option value="content"
										<c:if test="${projectPostVO.codeNo == 20001}">${'selected'}</c:if>>내용</option>
									<option value="2000101"
										<c:if test="${projectPostVO.codeNo == 2000101}">${'selected'}</c:if>>범죄
										&amp; 스릴러</option>
									<option value="2000102"
										<c:if test="${projectPostVO.codeNo == 2000102}">${'selected'}</c:if>>액션
										&amp; 어드벤쳐</option>
									<option value="2000103"
										<c:if test="${projectPostVO.codeNo == 2000103}">${'selected'}</c:if>>다큐
										&amp; 드라마/청춘</option>
									<option value="2000104"
										<c:if test="${projectPostVO.codeNo == 2000104}">${'selected'}</c:if>>역사
										&amp; 시대극</option>
									<option value="2000105"
										<c:if test="${projectPostVO.codeNo == 2000105}">${'selected'}</c:if>>판타지
										&amp; SF</option>
									<option value="2000106"
										<c:if test="${projectPostVO.codeNo == 2000106}">${'selected'}</c:if>>멜로
										&amp; 로맨스</option>
									<option value="2000107"
										<c:if test="${projectPostVO.codeNo == 2000107}">${'selected'}</c:if>>코미디</option>
									<option value="2000108"
										<c:if test="${projectPostVO.codeNo == 2000108}">${'selected'}</c:if>>애니메이션</option>
									<option value="2000109"
										<c:if test="${projectPostVO.codeNo == 2000109}">${'selected'}</c:if>>기타</option>
								</c:if>
						</select></td>
					</tr>
					...
					<tr>
						...
						<c:if test="${!empty projectPostVO.targetLimit }">
							<td><input type="date" name="targetLimit"
								value="${fn:substring(projectPostVO.targetLimit,0,10)  }" /></td>
						</c:if>
					</tr>

					<tr>
						<td id="fWrite_cont_document">리워드 필수 인증 서류<br />
							<p style="font-weight: initial;">서포터에게 제공할 리워드 (제품/서비스)의 생산 및
								유통 시, 필요한 인증 서류를 첨부해주세요.</p>
						</td>
					...
					<tr>
						<td id="fWrite_cont_Essential">리워드 필수 확인사항<br />
							<p style="font-weight: initial;">답변이 미흡할 경우, 심사 시 재요청 드리며 심사
								기간이 늘어날 수 있습니다.</p>
						</td>

						<td><b>Q1. 리워드가 타 크라우드펀딩사 및 온라인 커머스, 자사 홈페이지 등 다른 판매처에서
								유통된 적이 있거나 현재 유통 중인가요?</b><br /> 선택하신 답변이 사실과 다를 경우 약정서에 근거하여 프로젝트
							취소 및 위약벌이 부과될 수 있습니다. <br /> <br /> <input type="radio"
							name="warn" value="no" checked />아니요. 다른 곳에서 유통한 적이 없으며 와디즈를 통해
							처음 선보이는 제품입니다. <br /> <br /> <input type="radio" name="warn"
							value="yes" />예, 다른 곳에서 유통한 적이 있습니다. 또는 현재 유통 중입니다. <br /> <br /></td>
						<!--  <b>Q2. 현재 진행(제작)된
					...
					<tr>
						<td colspan="2"><textarea cols="133" rows="30" name="content"
       ...
       button_c9d8ce2 button_f12 button_p1024 button_r4" /></th>
		...
		<div id="fWrite_reward" class="menu-option">
			<%------첫번째 리워드------------------------------------------------ --%>
			<table border="1" id="fWrite_cont_reward_table" cellspacing="0">

				<tr>
					<%--테이블 tr , th 안에 테이블 넣기. 즉 중첩 테이블 --%>
					<th>
						<table border="1" id="fWrite_cont_reward_table2" cellspacing="0">
							<c:if test="${empty projectPostVO.rewardVO}">
								<c:forEach var="r" items="${projectPostVO.rewardVO }">
									<form method="post"
										action="/moving.com/rewards/edit_ok?id=${r.id }&where=reward"
										onsubmit="">
										<input type="hidden" name="projectPostId"
											value="${projectPostVO.id }"> <input type="hidden"
											name="id" value="${r.id }">
										<table border="1" id="fWrite_cont_reward_table2"
											cellspacing="0">
											<tr>
												<th rowspan="8">리워드 -</th>
												<td>금액</td>
												<td><input type="number" name="price" size="12"
													value="${r.price }" /> 원 <%-- 정렬순서 <input
								name="no" size="5" value="${r.no }"/> </td> --%>
											...
											<tr>
												<td>배송 조건</td>
												<td>배송료 <c:if test="${!empty r.deliveryFeeExistence }">
							...
							<input type="radio" name="deliveryFeeExistence" value="0"
															<c:if test="${r.deliveryFeeExistence == 0}">${'checked'}</c:if> />없음 
							</c:if>
												</td>
											</tr>
											<tr>
												<td>제한 수량</td>
												<td>리워드를 <input type="number" name="limitCount"
													size="12" value="${r.limitCount }" />개로 제한합니다.
												</td>
											</tr>
											<tr>
												<td>발송 시작일</td>
												<td><input type="date" name="deliveryExpectDate"
													value="${fn:substring(r.deliveryExpectDate,0,10)  }" /></td>
											</tr>
											<tr>
							...
							<div id="fWrite_reward1">
								<form method="post"
									action="/moving.com/rewards/write_ok?id=${projectPostVO.id }&where=reward"
									onsubmit="">
									<input type="hidden" name="projectPostId"
										value="${projectPostVO.id }">
									<table border="1">
										<tr>
											<th rowspan="8">리워드 추가)</th>
											<td>금액</td>
											<td style="padding-right: 115px;"><input type="number"
												name="price" size="12" value="0" style="text-align: right;" />
												원 <!-- 정렬순서 <input
								name="no" size="5" /> -->
												<!--  <input type="button" name="close" /> --></td>
										</tr>
										<tr>
											<td>리워드 명</td>
											<td><input name="title" size="41"
												placeholder="ex. [액션]내게 싸움은 살인이다." /></td>

										</tr>
										<tr>
											<td style="vertical-align: middle;">상세 설명</td>
											<td><textarea rows="10" cols="38" name="content"
													placeholder="영화의 대한 상세 설명을 작성해주세요."></textarea></td>
										</tr>
										...
										<tr>
											<td>제한 수량</td>
											<td style="padding-left: 30px;">리워드를 &nbsp; <input
												type="number" name="limitCount" size="12" value="0"
												style="text-align: right;" /> &nbsp;개로 제한합니다.
											</td>
										</tr>
										<tr>
											<td>발송 시작일</td>
											<td>발송 시작일</td>
											<td style="padding-left: 27px; text-align: left;"><input
												type="date" name="deliveryExpectDate" /></td>
										</tr>
										<tr>
											<th colspan="3"><input type="submit" name="save"
												value="저장하기" /></th>
							...
							<table>
								<tr height="50">
									<th colspan="3"><input type="button" value="다음 단계로>"
										onclick="$('html, body').stop().animate({scrollTop : 0}, 500);"
										id="nextButton2" data-tab="fWrite_director" />
								</tr>
							</table>
							</div>
							...
							<div id="fWrite_director" class="menu-option">
								<form method="post"
									action="/moving.com/project/write_business_ok?id=${projectPostVO.id }"
									onsubmit="" enctype="multipart/form-data">
									<input type="hidden" name="id" value="${projectPostVO.id }" />
									<table border="1">
										<tr>
											<th colspan="2">아래에 입력한 모든 정보는 프로젝트 페이지에 노출되는 정보이니,
												서포터와의 소통과 응대가 가능한 정확한 정보를 입력해주세요.</th>
										</tr>
										<tr>
											<th>
												<dl>
											...
											<td><input name="director" id="director" size="20"
												value="${business_name }" readonly="readonly" /></td>
										</tr>
										<tr>
											<th>
												<dl>
											...
											<td>
												<div class="select_profile_img">
													<c:if test="${'default' !=  profile_image_url  }">
														<img src="${profile_image_url}" width="100" height="100"
												...
												</div> <input type="file" name="profileImageUrl"
												id="profileImageUrl" /> <script>
													//파일 업로드 시 해당 화면에 바로 띄워 지게 해줌.
													$("#profileImageUrl")
															.change(
																	function() {
										...
										<tr>
											<td>
												<dl>
													<dt>소셜 네트워크</dt>
													<dd>서포터가 확인할 수 있는 메이커님의 페이스북, 인스타그램 링크 등을 등록해주세요.(최소
														1개 등록)</dd>
												</dl>
											</td>
											<td>
												<ul>
													<li><a
														href="https://192.168.0.81:8084/moving.com/social/profile?id=${sessionSocial.id}">
															무빙 SNS :
															https://192.168.0.81:8084/moving.com/social?id=${sessionSocial.id}</a></li>
													<li><a
														href="https://192.168.0.81:8084/moving.com/member_mypage?mid=${sessionSocial.id}">
															무빙 마이페이지 :
															https://192.168.0.81:8084/moving.com/member_mypage?mid=${sessionSocial.id}</a></li>
													<li><a
														href="https://192.168.0.81:8084/moving.com/social/messenger?socialIdFrom=${sessionSocial.id}&socialIdTo=${sessionSocial.id}">
															제작사에게 메시지 보내기 :
															https://192.168.0.81:8084/moving.com/social/messenger?socialIdFrom=${sessionSocial.id}&socialIdTo=${sessionSocial.id}</a></li>
												</ul>
											</td>
										</tr>
										<!-- <tr>
						...
						type="button" name="checkLink02" id="checkLink02" value="링크 확인" />
										...
										<tr>
											<th>
												<dl>
													<dt>함께하는 멤버 등록(선택)</dt>
													<dd>프로젝트를 함께 준비하시는 팀원분이 있다면 와디즈에 가입하신 계정을 등록해 주세요.
														프로젝트 상세 페이지 우측에 메이커분과 함께 노출됩니다.</dd>
												</dl>
											</th>
											<td><c:if test="${!empty projectPostVO.business}">
													<input name="withMem" id="withMem" size="30"
														oninput="nick_check2();"
														value="${projectPostVO.business }" />
												</c:if> <c:if test="${empty projectPostVO.business}">
													<input name="withMem" id="withMem" size="30"
														oninput="nick_check2();" />
												</c:if> <span id="addMem_nickcheck"></span> <!-- <script>
               /** 중복닉네임 체크 */
                ...
                --></td>
										...
										<tr>
											<td>
												<dl>
													<dt>문의 전화번호</dt>
													<dd>서포터 문의 시 실시간 연락이 가능하고, 프로젝트 페이지에 노출가능한 대표번호를
														입력해주세요.</dd>
												</dl>
											</td>
											<td><input name="phone2" id="phone2" size="30"
												value="${phone}" /></td>
										</tr>
										<!-- <tr>
					<th>
						...
						확인하실 수 있습니다.
										...
										<tr height="50">
											<th colspan="2"><input type="submit" value="저장하기" /> <input
												...
												onclick="$('html, body').stop().animate({scrollTop : 0}, 500);"
												id="nextButton3" data-tab="fWrite_calculate" /> <input
							...
							<div id="fWrite_calculate" class="menu-option">
								<table border="1">
									...
									<tr>
										...
										<td><input type="radio" name="company" value="personal"
											checked>개인 사업자 <input type="radio" name="company"
											value="company">법인 사업자 <br />
											<table>
												<tr>
													<td>사업자 번호</td>
													<td><input name="companyNo" id="companyNo" size="20"
														value="${business_register_no }" /></td>
												</tr>
												<tr>
													<td>사업자등록증</td>
													<td><c:if
															test="${!empty business_license_image_path }">
															<img src="${business_license_image_path}" width="100"
																height="150" />
														</c:if> <!-- <input type="file" id="companyVarif" /> --></td>
												</tr>
												<!-- <tr>
								<td>통장 사본 업로드</td>
								<td><input type="file" id="checkVarif" /></td>
									...
									<tr>
										<th><dl>
												<dt>계약정보 - 2) 대표자 정보</dt>
												<dd>
													전자 약정서는 대표자가 직접 확인하고, 서명을 진행해야함으로, <br /> 대표자의 이메일로 약정서가
													발송됩니다. 공동 대표의 경우는 모든 대표자의 메일로 발송되며, <br /> 모든 대표자가 확인하고
													서명하여야 약정이 체결됩니다.
												</dd>
											</dl></th>
										<td><input name="ceo" id="ceo" size="10" value="${name }" />
											<input name="ceoEmail" id="ceoEmail" size="20"
											value="${email }" /></td>
									</tr>
									...
									<tr height="50">
										<th colspan="3"><input type="submit" value="저장하기" /> <input
											type="button" value="다음 단계로>"
											onclick="$('html, body').stop().animate({scrollTop : 0}, 500);"
											id="nextButton4" data-tab="fWrite_preview" /> <input
											type="reset" value="초기화" /></th>
									</tr>
								</table>
							</div>
							...
							<div id="fWrite_preview" class="menu-option">
								...
								<div id="fCont_subtitle">
									...
									<div id="fCont_mainImage">
										...
										<c:if test="${projectPostVO.thumbnailImage != NULL}">
											<img src="${projectPostVO.thumbnailImage}" width="700"
												height="400" src="펀딩이미지05" />
										</c:if>
										...
										<div class="fCont_tags">
											<span class="sumCont_sub fCont_tag"><i
												...
												class="fas fa-tag fa-lg"></i> Documentary</span> <span
												class="sumCont_sub fCont_tag"><i
									...
									<div id="fCont_sumCont">
										<div id="fCont_sumContTS">
											...
											<span class="sumCont_title">${ projectPostVO.nowPrice }
												원</span> <span class="sumCont_sub">목표 금액
												${projectPostVO.targetPrice } 원 중</span> <span
												class="sumCont_title">${projectPostVO.sponserCount }
												명</span> <span class="sumCont_sub">프로젝트 후원자</span> <span
												class="sumCont_title">${projectPostVO.leftLimit }</span> <span
											...
											<span class="sumCont_sub"><a href="#">All or
													Nothing</a>. 이 프로젝트는 Sat, November 23 2019 1:59 PM UTC +09:00
												까지 목표금액 달성시에만 후원금이 전달됩니다.</span>
										</div>
									</div>
								...
								<div id="fCont_menubar">
									<div id="fCont_row">
										...
										<button class="fCont_menu" onclick="setScrollX(3750)">
											...
											<span class="commentCount"> <c:if
													test="${projectPostVO.commentCount != 0}">
									...
									<div id="fCont_scrollCont">
										...
										<div id="fCont_CampaignCont">
											...
											<c:if test="${!empty projectPostVO.attachedFileVO }">
												<c:forEach var="file"
													items="${projectPostVO.attachedFileVO }">
										...
										<div id="fCont_CommentsCont">
											<div class="SNS_Comment">
												...
												<div class="SNS_Comment_Write">
													...
													<input type="hidden" id="project_post_id" size="40"
														value="${projectPostVO.id }">
													<p id="sessionId">${nickname }</p>
													<input id="content" name="content"
														class="SNS_Comment_Write_Chat"> <input id="write"
														name="write" class="SNS_Comment_Write_Button"
