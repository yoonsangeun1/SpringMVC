...
public class ProjectCommentController {
	...
	public ResponseEntity<List<MCommentVO>> selectCommentList(@PathVariable("id") int id) {
		...
		try {
			entity=new ResponseEntity<>(this.projectPostService.selectCommentList(id), HttpStatus.OK);
		}catch(Exception e) { 
			e.printStackTrace(); 
	...
	public ResponseEntity<String> updateComment(
		...
		try {
			...
			this.projectPostService.updateProjectComment(mCommentVO);//댓글 수정
			entity=new ResponseEntity<String>("SUCCESS", HttpStatus.OK);
		}catch(Exception e) {
			e.printStackTrace();
			entity=new ResponseEntity<String>(e.getMessage(), HttpStatus.BAD_REQUEST);
	...
	public ResponseEntity<String> deleteComment(@PathVariable("id") int id) {
		...
		ResponseEntity<String> entity=null;
		try {
			this.projectPostService.deleteProjectComment(id);//댓글 삭제
	...
	public ResponseEntity<String> checkCommentAuthor(
			@RequestBody MCommentVO mCommentVO, 
			HttpServletRequest request, 
			HttpServletResponse response) throws Exception {
		 RequestBody는 전송된 JSON 데이터를 MCommentVO 타입의 객체로 변환  
